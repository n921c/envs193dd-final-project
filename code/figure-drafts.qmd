...
jitter <- ggplot(data = aq_ins_clean,
                 mapping = aes(x = taxon_list_clean,
                               y = log_ave_lit,
                               color = taxon)) +
  ...
  # coloring 
  scale_color_manual(values = taxon) +
...
figure4 <- ggplot(data = aq_ins_clean,
  ...
  facet_wrap(~season,
             scales = "free") +
  labs(title = "Seasonal Patterns Between Water Temperature and Log + 1 Abundance for Three Dominant Taxa",
